...
def Mget_lj_potential(D,  truncation_distance=box_len, truncation_value=0):
    ...
    potentials = 4 * energy_parameter * ((1/D) ** 12 - (1/D) ** 6) - truncation_value
...
##UPDATING - FUNCTIONS
def update_data(coordinates, orientations, potential_key, t):
...
def run():
    ...
    for t in range(its):
        ...
        coordinates, orientations, current_solutions = update_data(coordinates, orientations, potential_type, t)
    ...
    if plot_last_frame:
        ...
        #marker_size = (fig.dpi * fs * radius_to_image)**2 * np.pi

        for i in range(N):


            plt.scatter(np.asnumpy(centroids_x[i + (sample_its-1) * N]), np.asnumpy(centroids_y[i + (sample_its-1) * N]), color=[1,0,0], s=int(points_radius**2))
...
def run_again(coordinates, orientations):
    centroids_x = np.zeros((N * sample_its))
    centroids_y = np.zeros((N * sample_its))
    labels = np.arange(N * sample_its,
                       dtype=np.int64)  # Set this to this since label is assumed to not matter for the moment
    ...
    if plot:
        ...
        for i in range(N):
            for t in range(0, its, int(its / 40)):
                if t == 0:
                    plt.scatter(all_centroids_x[i + t * N], all_centroids_y[i + t * N], color=color_v[i], s=40,
                                alpha=0.5,
                                marker='*')
                else:
                    plt.scatter(all_centroids_x[i + t * N], all_centroids_y[i + t * N], color=color_v[i], s=4,
                                alpha=0.5)
...
coordinates, orientations = run()
a = input("Write C to continue...")
while a == 'C':
    coordinates, orientations = run_again(coordinates, orientations)
